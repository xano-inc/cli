...
├── policy-command.ts            # Base class for `policy *` commands and `skills pull` (shared flags, target, source, exit 1 on failure)
...
│   ├── skills/                  # Agent skills generated by the instance (pull)
...
| `PolicyCommand` | `src/policy-command.ts` | `policy *` commands and `skills pull` - shared `-w/-b/-o` flags, `policyTarget(flags, route?)` (workspace, branch, request), `readSource`; each command owns its `run()`; operational errors exit 1 |
