...
xano function run <name> --data_source test             # run against the 'test' data source
```
...
`--data_source <label>` (alias `--datasource`) runs the function against a non-live data
source by sending the `X-Data-Source` header, so table reads and writes hit that data
source's tables. Omit it to run against `live`; an unknown label is rejected by the server
with `Invalid data source.`
