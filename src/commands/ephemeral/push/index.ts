...
export default class EphemeralPush extends BaseCommand {
  ...
  static override examples = [
    `$ xano ephemeral push -t e4f2-9ab1-xyz1
Push from current directory (default partial mode)
`,
    `$ xano ephemeral push -t e4f2-9ab1-xyz1 -d ./my-workspace
Push from a specific directory
`,
    `$ xano ephemeral push -t e4f2-9ab1-xyz1 --sync
Push all files to the ephemeral tenant
`,
    `$ xano ephemeral push -t e4f2-9ab1-xyz1 --sync --delete
Push all files and delete remote objects not included
`,
    `$ xano ephemeral push -t e4f2-9ab1-xyz1 --dry-run
Preview changes without pushing
`,
    `$ xano ephemeral push -t e4f2-9ab1-xyz1 --force
Skip preview and push immediately
`,
    `$ xano ephemeral push -t e4f2-9ab1-xyz1 --records --env`,
    `$ xano ephemeral push -t e4f2-9ab1-xyz1 --truncate`,
  ]
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(EphemeralPush)
