...
export default class EphemeralStaticHostBuildPull extends BaseCommand {
...
`,
  ...
  async run(): Promise<void> {
    const {args, flags} = await this.parse(EphemeralStaticHostBuildPull)
  ...
  private async resolveEnvBuild(opts: {
    ...
    const {env, profile, staticHost, tenantName, verbose, workspaceId} = opts
    // 1. Look up the static host to read the deployed canonical for this env.
    const hostUrl = `${profile.instance_origin}/api:meta/workspace/${workspaceId}/tenant/${tenantName}/static_host`
    ...
    const perPage = 100
    for (let page = 1; ; page++) {
      const listUrl = `${profile.instance_origin}/api:meta/workspace/${workspaceId}/tenant/${tenantName}/static_host/${staticHost}/build?page=${page}&per_page=${perPage}`
  ...
  private async resolveLatestBuild(
    ...
    const listUrl = `${profile.instance_origin}/api:meta/workspace/${workspaceId}/tenant/${tenantName}/static_host/${staticHost}/build?page=1&per_page=1`
