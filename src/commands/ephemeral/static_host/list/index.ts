...
export default class EphemeralStaticHostList extends BaseCommand {
...
static description = 'List all static hosts for an ephemeral tenant from the Xano Metadata API'
static examples = [
    `$ xano ephemeral static_host list e4f2-9ab1-xyz1 -w 40
Available static hosts:
  - my-static-host (ID: 1)
  - another-host (ID: 2)
`,
    `$ xano ephemeral static_host list e4f2-9ab1-xyz1 --profile production
Available static hosts:
  - my-static-host (ID: 1)
  - another-host (ID: 2)
`,
    `$ xano ephemeral static_host list e4f2-9ab1-xyz1 -w 40 --output json
{
  "count": 1,
  "page": 1,
  "total": 3,
...
`,
    `$ xano ephemeral static_host list e4f2-9ab1-xyz1 -p staging -o json --page 2
[
  {
    "id": 3,
    "name": "static-host-3"
...
static override flags = {
  ...
  async run(): Promise<void> {
    const {args, flags} = await this.parse(EphemeralStaticHostList)
    ...
    if (flags.workspace) {
      ...
      this.error(
        ...
        `  1. Provide it as a flag: xano ephemeral static_host list <tenant_name> -w <workspace_id>\n` +
    ...
    const apiUrl = `${profile.instance_origin}/api:meta/workspace/${workspaceId}/tenant/${tenantName}/static_host?${queryParams.toString()}`
