...
export default class McpServerTriggerEdit extends BaseCommand {
  static override description = 'Edit an existing MCP server trigger'
  ...
  async run(): Promise<void> {
    const {args, flags} = await this.parse(McpServerTriggerEdit)
    try {
      ...
      const trigger = await client.updateMcpServerTrigger(workspaceId, args.mcp_server_id, args.trigger_id, triggerData)
