...
export default class RealtimeChannelCreate extends BaseCommand {
  static override description = 'Create a new realtime channel'
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(RealtimeChannelCreate)
    try {
      ...
      if (flags.file) {
        ...
        channelData = {name: flags.name}
      } else {
        this.error('Either --name, --data, or --file must be provided')
      }
      const channel = await client.createRealtimeChannel(workspaceId, channelData) as {id: number}
      if (flags.output === 'json') {
        this.log(JSON.stringify(channel, null, 2))
      } else {
        this.log('Channel created successfully!')
        this.log(`ID: ${channel.id}`)
      }
    } catch (error) {
