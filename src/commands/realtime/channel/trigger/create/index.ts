...
export default class RealtimeChannelTriggerCreate extends BaseCommand {
  static override description = 'Create a new trigger for a realtime channel'
  ...
  async run(): Promise<void> {
    const {args, flags} = await this.parse(RealtimeChannelTriggerCreate)
    try {
      ...
      const trigger = await client.createRealtimeChannelTrigger(workspaceId, args.channel_id, triggerData) as {id: number}
