...
export default class TableIndexCreateUnique extends BaseCommand {
  static override description = 'Create a unique index on a table'

  static override examples = [
    `$ xano table index create unique 123 -w 40 --columns email
Unique index created successfully!
`,
    `$ xano table index create unique 123 -w 40 -f index.json -o json
{"id": 456, "type": "unique", ...}
`,
  ]
  ...
  async run(): Promise<void> {
    const {args, flags} = await this.parse(TableIndexCreateUnique)
    ...
    try {
      ...
      const result = await client.createTableIndexUnique(workspaceId, args.table_id, indexData)
