...
export default class WorkflowTestEdit extends BaseCommand {
  static override description = 'Edit an existing workflow test'
  ...
  async run(): Promise<void> {
    const {args, flags} = await this.parse(WorkflowTestEdit)
    try {
      ...
      if (flags.file) {
        ...
        testData = {}
        if (flags.name) testData.name = flags.name
        if (flags.description !== undefined) testData.description = flags.description
        if (Object.keys(testData).length === 0) this.error('At least one of --name, --description, --data, or --file must be provided')
      }
      const test = await client.updateWorkflowTest(workspaceId, args.workflow_test_id, testData)
      if (flags.output === 'json') {
        this.log(JSON.stringify(test, null, 2))
      } else {
        this.log('Workflow test updated successfully!')
      }
    } catch (error) {
