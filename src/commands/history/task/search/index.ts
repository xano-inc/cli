...
export default class TaskHistorySearch extends BaseCommand {
  static override description = 'Search task execution history'

  static override flags = {
    ...BaseCommand.baseFlags,
    workspace: Flags.string({char: 'w', description: 'Workspace ID'}),
    'task-id': Flags.integer({description: 'Filter by task ID'}),
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const response = await client.searchTaskHistory(workspaceId, searchParams) as {items?: unknown[]} | unknown[]
      const items = Array.isArray(response) ? response : ((response as {items?: unknown[]})?.items || [])

      this.log(flags.output === 'json' ? JSON.stringify(items, null, 2) : `Found ${items.length} task history entries`)
    } catch (error) {
