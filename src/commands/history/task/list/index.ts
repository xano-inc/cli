...
export default class TaskHistoryList extends BaseCommand {
  static override description = 'List task execution history for a workspace'
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(TaskHistoryList)
    try {
      const client = XanoApiClient.fromProfile(flags.profile || XanoApiClient.getDefaultProfile())
      const workspaceId = client.getWorkspaceId(flags.workspace)

      const response = await client.listTaskHistory(workspaceId, {
      ...
      this.log(flags.output === 'json' ? JSON.stringify(items, null, 2) :
        items.length === 0 ? 'No task history found' : `Found ${items.length} task history entries`)
    } catch (error) {
