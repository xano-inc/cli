...
export default class FunctionHistorySearch extends BaseCommand {
  static override description = 'Search function execution history'

  static override flags = {
    ...BaseCommand.baseFlags,
    workspace: Flags.string({char: 'w', description: 'Workspace ID'}),
    'function-id': Flags.integer({description: 'Filter by function ID'}),
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(FunctionHistorySearch)
    try {
      const client = XanoApiClient.fromProfile(flags.profile || XanoApiClient.getDefaultProfile())
      const workspaceId = client.getWorkspaceId(flags.workspace)

      const searchParams: Record<string, unknown> = {}
      if (flags['function-id']) searchParams.function_id = flags['function-id']
      ...
      const response = await client.searchFunctionHistory(workspaceId, searchParams) as {items?: unknown[]} | unknown[]
      const items = Array.isArray(response) ? response : ((response as {items?: unknown[]})?.items || [])

      this.log(flags.output === 'json' ? JSON.stringify(items, null, 2) : `Found ${items.length} function history entries`)
    } catch (error) {
