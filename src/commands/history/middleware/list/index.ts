...
export default class MiddlewareHistoryList extends BaseCommand {
  static override description = 'List middleware execution history for a workspace'
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(MiddlewareHistoryList)
    try {
      const client = XanoApiClient.fromProfile(flags.profile || XanoApiClient.getDefaultProfile())
      const workspaceId = client.getWorkspaceId(flags.workspace)

      const response = await client.listMiddlewareHistory(workspaceId, {
      ...
      this.log(flags.output === 'json' ? JSON.stringify(items, null, 2) :
        items.length === 0 ? 'No middleware history found' : `Found ${items.length} middleware history entries`)
    } catch (error) {
