...
export default class ToolHistoryList extends BaseCommand {
  static override description = 'List tool execution history for a workspace'
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(ToolHistoryList)
    try {
      const client = XanoApiClient.fromProfile(flags.profile || XanoApiClient.getDefaultProfile())
      const workspaceId = client.getWorkspaceId(flags.workspace)

      const response = await client.listToolHistory(workspaceId, {
      ...
      this.log(flags.output === 'json' ? JSON.stringify(items, null, 2) :
        items.length === 0 ? 'No tool history found' : `Found ${items.length} tool history entries`)
    } catch (error) {
