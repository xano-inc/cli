...
export default class ToolHistorySearch extends BaseCommand {
  static override description = 'Search tool execution history'

  static override flags = {
    ...BaseCommand.baseFlags,
    workspace: Flags.string({char: 'w', description: 'Workspace ID'}),
    'tool-id': Flags.integer({description: 'Filter by tool ID'}),
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const response = await client.searchToolHistory(workspaceId, searchParams) as {items?: unknown[]} | unknown[]
      const items = Array.isArray(response) ? response : ((response as {items?: unknown[]})?.items || [])

      this.log(flags.output === 'json' ? JSON.stringify(items, null, 2) : `Found ${items.length} tool history entries`)
    } catch (error) {
