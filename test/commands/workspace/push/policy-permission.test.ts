 ...
 */
describe('workspace push with policy files, without the workspace:policy permission', () => {
  ...
  const message = 'The workspace:policy update permission is required on this workspace; nothing was imported: AUTH-001'
  const refused = (code = 'policy_permission_required') =>
    json({code: 'ERROR_CODE_ACCESS_DENIED', message, payload: {code, level: 'update', permission: 'workspace:policy', policies: ['AUTH-001']}}, 403)
  ...
  it('explains a refused policy change instead of repeating it, and imports nothing', async () => {
    ...
    expect(result.error?.message).to.contain(`Push refused (403): ${message}`)
    expect(result.error?.message).to.contain('-e "policies/*"')
    expect(result.error?.message).to.contain('unchanged never cause this')
    expect(result.error?.message).to.contain('changing them needs the `workspace:policy` update permission')
    expect(result.error?.message).to.contain('ask someone with that permission')
    expect(result.error?.message).not.to.contain('Manage Access Tokens')
  })

  it('sends a token without the scope to create one that has it', async () => {
    fixture.route(() => refused('policy_scope_required'))
    const result = await push('--force')
    expect(result.error?.oclif?.exit).to.equal(1)
    expect(result.error?.message).to.contain('changing them needs the `workspace:policy` update scope on your token')
    expect(result.error?.message).to.contain('-e "policies/*"').and.to.contain('Manage Access Tokens')
  })

  it('does not explain a refusal that names no policy files as a policy-file refusal', async () => {
    fixture.route(() => json({message: 'Policy files require the workspace:policy permission; nothing was imported: AUTH-001'}, 403))
    const result = await push('--force')
    expect(result.error?.oclif?.exit).to.equal(1)
    expect(result.error?.message).not.to.contain('-e "policies/*"')
  })
